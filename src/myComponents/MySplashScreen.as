package myComponents
{
	...
	/**
	 *  The MySplashScreen class is the default preloader for Mobile Flex applications.
	 *
	 ...
	 *
	 *  The MySplashScreen monitors device orientation and updates the image so that it
	 ...
	 */
	public class MySplashScreen extends Sprite implements IPreloaderDisplay
	{
		//--------------------------------------------------------------------------
		//
		//  Constructor
		 ...
		 *  @productversion Flex 4.5
		 */
		public function MySplashScreen()
		{
			super();
		}
		...
		/**
		 * if set to 0 while splash screen is shown, then splashscreen will be removed<br
		 * 1000 is the value used if no value is set in HelpDiabetes.mxml<br>
		 */
		static private var _displayTime:Number = 1000;
		static private var _timeInBetweenTwoLongShowsOfSplashScreen:Number = (5 * 24 + 7) * 3600 * 1000;//arbitraty value, a bit more than 5 days
		
		/**
		 * to be called as soon as startup is finished, and splash screen can be removed.<br>
		 * should only be called if we're sure that the database exists, can be verifed with Database.getinstance().databaseexists()
		 */
		public static function resetDisplayTime():void
		{
			var lastTimeShown:Number = new Number(Settings.getInstance().getSetting(Settings.SettingsTimeStampOfLastTimeSplashScreenWasShownForLongTime));
			if (lastTimeShown == 0) {
				//actually it's the first time we come here, means the second time the app was started on this device
				//so we will not show the splashscreen but we simply set the setting to now
				Settings.getInstance().setSetting(Settings.SettingsTimeStampOfLastTimeSplashScreenWasShownForLongTime,
					( (
						(new Date()).valueOf() 
					).toString()
					)
				);
				lastTimeShown = new Number(Settings.getInstance().getSetting(Settings.SettingsTimeStampOfLastTimeSplashScreenWasShownForLongTime));
				_displayTime = 0;
			} else {
				if ((new Date()).valueOf() - lastTimeShown > _timeInBetweenTwoLongShowsOfSplashScreen) {
					//we continue to show the splashscreen and we set new value
					Settings.getInstance().setSetting(Settings.SettingsTimeStampOfLastTimeSplashScreenWasShownForLongTime,
						( (
							(new Date()).valueOf() 
						).toString()
       ...
       // in ms
			...
			if (imageClass && imageClass != SplashImageClass)
			{
				...
				if (!SplashImageClass)
				{
					...
					if ("splashScreenMinimumDisplayTime" in info) {
						...
						_displayTime = this.minimumDisplayTime;
					}
		...
		private function Stage_resizeHandler(event:Event):void
		{
			...
			switch(scaleMode)
			{
				...
				case "none":
					// SDK-30984: undo application's dpi scaling if we have a dynamic MySplashScreen source
					if (dynamicSource)
					{
						scaleX = 1 / dpiScale;
						scaleY = 1 / dpiScale;
		...
		{
			if (currentDisplayTime <= minimumDisplayTime && _displayTime > 0)
				return;
